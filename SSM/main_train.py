...
from SSM.models import TrajectoryS4Model
#################################################
...
print('Intializing SSM Model\n')
# model = TrajectoryS4dModel(state_dim=n_state, act_dim=n_action)      #S4d model - minimal relaization
model = TrajectoryS4Model(state_dim=n_state, act_dim=n_action)      #S4 model full realization
...
print('\n======================')
print('Initializing training..yoyo\n')
        ...
        if step>= 2:      #100000
            ...
            with accelerator.accumulate(model):
                ...
                if (step % eval_steps*10) == 0:
                    accelerator.save_state(root_folder + '/SSM/saved_files/checkpoints_S4D_l24_d64/checkpoint_rtn_s4d_train')
...
plt.plot(eval_steps, eval_loss_values, label='Evaluation Loss', linestyle='--')
plt.xlabel('Steps')
plt.ylabel('Loss')
