...
This code is utilized form the https://github.com/state-spaces/s4/blob/main/models/s4
"""
import os
import sys
import argparse
...
'''
I am currently using this single gpu logger to avoid installation of pytorch lightening
'''
...
def get_logger(name=__name__, level=logging.INFO) -> logging.Logger:
    ...
    # Normal logger without distributed check
    return logger
...
def dplr(
    init='hippo',
    N=512, rank=1, H=1,   # Changing d_state was 64/128/256/512
    dtype=torch.float,
    real_random=False,
    real_scale=1.0,
    imag_random=False,
...
class SSMKernel(Kernel):
    ...
    def __init__(
        ...
        verbose: bool = True,
        # SSM arguments
        d_state: int = 512,    # Changing d_state was 64/128/256/512
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        d1_time = time.time()
        # print(f' The tim1 betwen enter and dtc calculation: {d1_time - time_en}')
        ...
        time_cauchbef = time.time()
        
        # print(f'INSIJDE SSM KERNEL DPLR: before cauchy time : {time_cauchbef - time_en}')
        r = cauchy_mult(v, z, A)
    ...
    def _step_state(self, u, state):
        ...
        # print(f"Shape of self.dB: {self.dB.shape}")
        # print(f"Shape of u: {u.shape}")
        # print(f"Input contraction equation: {self.input_contraction}")
...
class FFTConv(nn.Module):
    ...
    def forward(self, x, state=None, rate=1.0, **kwargs): # absorbs return_output and transformer src mask
        ...
        time_ssk = time.time()
        # print(f'The time taken to pass between enter and sskernek {time_ssk-time_en}')
...
class S4Block(nn.Module):
    ...
    def forward(self, x, lengths=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        # print(f'The time taken for passing through FFT conv layer (1/12): {FFT_time}')
