...
'''
######### Initialize dataset objects and dataloader ##############
'''
...
config = MambaConfig(
    ...
    d_model=600,     # Hidden dimension size 384/250/128
...
'''
Model Initilization and Training part
'''
...
'''
Averaged out evaluation of intermediate trained datatset 
It is evaluated and averaged out over 100 sample points
'''
    ...
    for step, batch in enumerate(train_dataloader, start=0):
        if step>= 56000: 
            ...
            with accelerator.accumulate(model):
                ...
                '''
                This particular training setup is for model that takes
                actions as input and outputs the state
                Essentially this is to learn the dynamical model of the system
                '''
                state_preds = model(
                    states=states_i,
                    actions=actions_i,
                ...
                loss = loss_i_state  # Add loss_i_action in other cases
                if step % 10 == 0:
                    accelerator.print(
                        {
                ...
                if (step % (eval_steps)) == 0:
                    eval_loss, loss_state = evaluate()
                    accelerator.print({"loss/eval": eval_loss, "loss/state": loss_state})
                ...
                if (step % eval_steps*10) == 0:
                    accelerator.save_state(root_folder + '/mamba/saved_files/checkpoints_S64_l6_d128_dm512_test/checkpoint_rtn_s6_train')
...
'''
Generating all the plots for the trained and evaluated model
...
plt.legend()
plt.grid(True)
plt.savefig(root_folder + '/mamba/saved_files/checkpoints_S64_l6_d128_dm512_test/trainloss.png')
plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig(root_folder + '/mamba/saved_files/checkpoints_S64_l6_d128_dm512_test/evalloss.png')
plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig(root_folder + '/mamba/saved_files/checkpoints_S64_l6_d128_dm512_test/evalstateloss.png')
plt.show()
...
file_name = "checkpoints_S64_l6_d128_dm512_test_losses_data.csv"
...
with open(file_name, mode="w", newline="") as file:
    ...
    # Write header
    writer.writerow(["Step", "Train_Loss", "Eval_Step", "Eval_Loss", "State_Loss"])
    # Write rows
