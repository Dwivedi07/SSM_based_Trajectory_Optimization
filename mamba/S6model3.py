...
class DeepMambaModel(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        """
        # super(DeepMambaModel, self).__init__()
        self.config = config
        ...
        self.embed_state = nn.Linear(self.n_state, int(self.d_model/2))
        self.embed_action = nn.Linear(self.n_action, int(self.d_model/2))
        self.embed_return = nn.Linear(1, int(self.d_model/2))  # Returns-to-go
        self.embed_constraint = nn.Linear(1, int(self.d_model/2))  # Constraints-to-go
        self.embed_timestep = nn.Embedding(self.max_ep_len, int(self.d_model/2))  # Time embeddings
    ...
    def forward(self, states, actions, returns_to_go, constraints_to_go, timesteps, inference_params=None):
        ...
        # state_embeddings = state_embeddings + return_embeddings + constraint_embeddings
        # following will make the sequence look like ([s'_1 a_1], [s'_2, a_2], ...)
        # which works nice in an autoregressive sense since states predict actions
        stacked_inputs = (
            torch.stack((state_embeddings, action_embeddings), dim=2)
            .reshape(batch_size, seq_length, 2*( int(self.d_model/2)))
        )        
        
        # Mamba block takes: (B, L, D) format where we are passing the added tokens of actions, constraints, and returns
        ...
        # Apply normalization
        outputs = self.norm(x)
        # outputs = x.reshape(batch_size, seq_length, 2,  int(self.d_model/2)) # Repermutation of the data
        
        # Predict next action and next state
        state_preds = self.predict_state(outputs)   # predict next state given state and action
    ...
    def step(self, states, actions, returns_to_go, constraints_to_go, timesteps, conv_states, ssm_states, inference_params=None):
        ...
        stacked_inputs = (
            torch.stack((state_embeddings, action_embeddings), dim=2)
            .reshape(batch_size, 2*( int(self.d_model/2)))
        )
        ...
        return state_preds, conv_states, ssm_states
