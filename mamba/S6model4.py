...
class Mamba_Traj(nn.Module):
    def __init__(
        ...
        action_tanh=True,
        reward_type='normal',
        time_embd=True,  #Initially it was fasle
        type_input='B4LD',
    ):
        super().__init__()
    ...
    def forward(self, states, actions, returns_to_go, constraints_to_go, timesteps, running=False, cache=None, goals=None, target_goal=None,
        ...
        if self.type_input == 'B4LD':
            u = torch.stack(
                (returns_embeddings, constraints_embeddings, action_embeddings, state_embeddings), dim=1
            ).permute(0, 2, 1, 3).reshape(batch_size, 4*(sequence_len-del_r), self.input_emb_size)
        elif self.type_input == 'BL4D':
            u = torch.cat([returns_embeddings, constraints_embeddings, action_embeddings, state_embeddings], dim=-1)

        y = self.backbone(u)
        y = y.reshape(batch_size, sequence_len-del_r, 4, self.d_model).permute(0, 2, 1, 3)
    ...
    def get_action_RNN(self, states, actions, returns_to_go, constraints_to_go, timesteps, position_ids=None, num_last_tokens=0, **kwargs):
        ...
        if self.type_input == 'B4LD':
            u = torch.stack(
                (returns_embeddings, constraints_embeddings, action_embeddings, state_embeddings), dim=1
            ).permute(0, 2, 1, 3).reshape(bs, 4, self.input_emb_size)
        elif self.type_input == 'BL4D':
            u = torch.cat([returns_embeddings, constraints_embeddings, action_embeddings, state_embeddings], dim=-1)
        ...
        if self.type_input == 'B4LD':
            outputs = outputs.reshape(bs, 1, 4, self.d_model).permute(0, 2, 1, 3)

        state_preds = self.predict_state(outputs[:,-2]) 
        action_preds = self.predict_action(outputs[:,-1])
        # print('The shape of action and state,', action_preds.shape, state_preds.shape)
        
        return state_preds, action_preds


    def get_action_TWindow(self, states, actions, returns_to_go, constraints_to_go, timesteps, **kwargs):
        ...
        state_preds, action_preds= self.forward(
            states, actions, returns_to_go, constraints_to_go, timesteps, running=True, **kwargs)
