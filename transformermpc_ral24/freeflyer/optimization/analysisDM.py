...
def for_computation(input_iterable):
    ...
    # if np.char.equal(feas_cvx,'optimal') and mdp_constr:
    #     if transformer_ws == 'dyn':
    #         DT_S4D_trajectory, runtime_DT_S4D = DT_manager.torch_model_inference_dyn(model_S4D, test_loader, test_sample, rtg_perc=None, ctg_perc=0., rtg=rtg_0, ctg_clipped=True)
    #     elif transformer_ws == 'ol':
    #         DT_S4D_trajectory, runtime_DT_S4D = DT_manager.torch_model_inference_ol(model_S4D, test_loader, test_sample, rtg_perc=None, ctg_perc=0., rtg=rtg_0, ctg_clipped=True)
    # else:
    #     if transformer_ws == 'dyn':
    #         DT_S4D_trajectory, runtime_DT_S4D = DT_manager.torch_model_inference_dyn(model_S4D, test_loader, test_sample, rtg_perc=1., ctg_perc=0., rtg=None, ctg_clipped=True)
    #     elif transformer_ws == 'ol':
    #         DT_S4D_trajectory, runtime_DT_S4D = DT_manager.torch_model_inference_ol(model_S4D, test_loader, test_sample, rtg_perc=1., ctg_perc=0., rtg=None, ctg_clipped=True)    
...
if __name__ == '__main__':
    ...
    for i, res in enumerate(tqdm(p.imap(for_computation, zip(np.arange(N_data_test), itertools.repeat(other_args))), total=N_data_test)):
        ...
        if i % 1000 == 0:
            ...
            np.savez_compressed(root_folder + '/optimization/saved_files/warmstarting_S6FULL/wsF_analysis_' + transformer_model_name + '_vs_' + ssm_model_name + '_test'+ str(i),
    ...
    #  Save dataset (local folder for the workstation)
    np.savez_compressed(root_folder + '/optimization/saved_files/warmstarting_S6FULL/wsF_analysis_' + transformer_model_name + '_vs_' + ssm_model_name + '_test',
                        J_vect_scp_cvx = J_vect_scp_cvx,
                        # J_vect_scp_S4D = J_vect_scp_S4D,
                        J_vect_scp_DT = J_vect_scp_DT,
                        J_vect_scp_S6 =J_vect_scp_S6,
