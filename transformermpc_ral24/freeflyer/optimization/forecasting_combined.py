...
if __name__ =='__main__':
    ...
    model_S4 = DMS4_manager.get_S4_model(ssm_model_name4, train_loader, eval_loader)
    ...
    feasibility_list_S4 = np.full(N_data_test, False, dtype=bool)
    ...
    J_list_S4 = np.zeros(N_data_test, dtype=float)
    ...
    J_vect_scp_list_S4 = np.zeros(shape=(N_data_test,), dtype=object)  # For storing arrays
    ...
    runtime_list_S4 = np.zeros(N_data_test, dtype=float)
    ...
    runtime_scp_list_S4 = np.zeros(N_data_test, dtype=float)
    ...
    for idx, test_sample in enumerate(test_loader):
        ...
        inference_func_S4 = getattr(DMS4_manager, 'ssm_model_inference_'+transformer_ws+'_T') 
        ...
        ################################################ S4
        S4_trajectory, runtime_S4 = inference_func_S4(model_S4, test_loader, test_sample, rtg_perc=1., ctg_perc=0., rtg=rtg, ctg_clipped=True)
        out['runtime_S4'] = runtime_S4
        out['J_S4'] = np.sum(la.norm(S4_trajectory['dv_' + transformer_ws], ord=1, axis=0))
        # Warm start states & actions for S4
        states_ws_S4 = np.append(S4_trajectory['xypsi_' + transformer_ws], (S4_trajectory['xypsi_' + transformer_ws][:,-1] + ffm.B_imp @ S4_trajectory['dv_' + transformer_ws][:, -1]).reshape((6,1)), 1)# set warm start
        actions_ws_S4 = S4_trajectory['dv_' + transformer_ws]

        # Solve SCP for ART
        runtime0_scp_S4 = time.time()
        traj_scp_S4, J_vect_scp_S4, iter_scp_S4, feas_scp_S4 = ocp_obstacle_avoidance(ffm, states_ws_S4, actions_ws_S4, state_init, state_final)
        runtime_scp_S4 = time.time() - runtime0_scp_DT
        
        
        if np.char.equal(feas_scp_S4,'optimal'):
            # Save scp_S4 in the output dictionary
            # Computing RMSE
            states_scp_S4, actions_scp_S4 = traj_scp_S4['states'], traj_scp_S4['actions_G']
            states_scp_S4 = states_scp_S4[:, :-1]
            out['feasible_S4'] = True
            out['J_vect_scp_S4'] = J_vect_scp_S4
            out['iter_scp_S4'] = iter_scp_S4
            out['runtime_scp_S4'] = runtime_scp_S4

            # Compute errors
            out['trajectory_rmse'] = np.sqrt(np.mean((states_scp_S4 - states_i_np) ** 2))
            out['control_error'] = np.sqrt(np.mean((actions_scp_S4 - actions_i_np) ** 2))
            out['final_state_error'] = np.sqrt(np.mean((states_scp_S4[:, -1] - goal_i_np[:, -1]) ** 2))
            

        else:
            out['feasible_S4'] = False
        
        ''' Store results in preallocated arrays '''
        trajectory_rmse_list_S4[idx] = out.get('trajectory_rmse', np.nan)
        control_error_list_S4[idx] = out.get('control_error', np.nan)
        final_state_error_list_S4[idx] = out.get('final_state_error', np.nan)
        feasibility_list_S4[idx] = out.get('feasible_S4')
        runtime_list_S4[idx] = out.get('runtime_S4', np.nan)
        runtime_scp_list_S4[idx] = out.get('runtime_scp_S4', np.nan)
        iter_scp_list_S4[idx] = out.get('iter_scp_S4', 20000)
        J_vect_scp_list_S4[idx] = out.get('J_vect_scp_S4', np.array([]))
        J_list_S4[idx] = out.get('J_S4', np.nan)
        ###############################################
        ...
        if type_model_ == 'gsa':
            ...
            if states_scp_cvx is not None:
                p1 = ax.plot(states_scp_cvx[0, :], states_scp_cvx[1, :], color='royalblue', linewidth=1.5, label='scp-cvx', zorder=3)
            p2 = ax.plot(states_ws_DT[0, :], states_ws_DT[1, :], color='darkred', linestyle='dashed', linewidth=1.5, label='warm-start ART-' + transformer_ws, zorder=3)
            p3 = ax.plot(states_ws_S6[0, :], states_ws_S6[1, :], color='darkgreen', linestyle='dashed', linewidth=1.5, label='warm-start S6-' + transformer_ws, zorder=3)
            p4 = ax.plot(states_ws_S4[0, :], states_ws_S4[1, :], color='purple', linestyle='dashed', linewidth=1.5, label='warm-start S4D-' + transformer_ws, zorder=3)
            ...
            plt.savefig(f"{root_folder}/optimization/saved_files/prediction_analysis_combined/plots/pos_3d_{idx}.png" )
        
        else:
            ax = plt.figure(figsize=(12,8)).add_subplot()
            # Warm-start CVX
            p1 = ax.plot(states_ws_cvx[0, :], states_ws_cvx[1, :], color='royalblue', linestyle='dashed', linewidth=1.5, label='warm-start cvx', zorder=3)
            if states_scp_cvx is not None:
                p2 = ax.plot(states_scp_cvx[0, :], states_scp_cvx[1, :], color='royalblue', linewidth=1.5, label='scp-cvx', zorder=3)

            # Warm-start ART 
            p3 = ax.plot(states_ws_DT[0, :], states_ws_DT[1, :], color='darkred', linestyle='dashed', linewidth=1.5, label='warm-start ART-' + transformer_ws, zorder=3)
            if states_scp_DT is not None:
                p6 = ax.plot(states_scp_DT[0, :], states_scp_DT[1, :], color='darkred', linewidth=1.5, label='scp-ART-' + transformer_ws, zorder=3)

            # Warm-start S6 
            p4 = ax.plot(states_ws_S6[0, :], states_ws_S6[1, :], color='darkgreen', linestyle='dashed', linewidth=1.5, label='warm-start S6-' + transformer_ws, zorder=3)
            if states_scp_S6 is not None:
                p7 = ax.plot(states_scp_S6[0, :], states_scp_S6[1, :], color='darkgreen', linewidth=1.5, label='scp-S6-' + transformer_ws, zorder=3)

            # Warm-start S4D )
            p5 = ax.plot(states_ws_S4[0, :], states_ws_S4[1, :], color='purple', linestyle='dashed', linewidth=1.5, label='warm-start S4D-' + transformer_ws, zorder=3)
            if states_scp_S4 is not None:
                p8 = ax.plot(states_scp_S4[0, :], states_scp_S4[1, :], color='purple', linewidth=1.5, label='scp-S4D-' + transformer_ws, zorder=3)
            ...
            ax.grid(True)
            ax.legend(loc='best', fontsize=10)
            plt.savefig(f"{root_folder}/optimization/saved_files/prediction_analysis_combined/plots_rc/pos_3d_{idx}.png" )
        

        if idx % 1000 == 0:
            ''' Save results '''
            np.savez_compressed(root_folder + '/optimization/saved_files/prediction_analysis_combined/c1/pred_analysis_c_' + transformer_model_name + '_vs_' + ssm_model_name + '_test'+str(idx),
                    # Feasibility flags
                    feasibility_list_CVX = feasibility_list_CVX,
                    feasibility_list_DT = feasibility_list_DT,
                    feasibility_list_S6 = feasibility_list_S6,
                    feasibility_list_S4 = feasibility_list_S4,
