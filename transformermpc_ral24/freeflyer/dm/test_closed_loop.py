...
model_name = 'checkpoint_ff_S6_rcgsa' #'checkpoint_ff_S6_gsa', 'checkpoint_ff_S6_rcgsa'
mdp_constr = True
...
'''
Loading the dataset
1) DT manager: if the model is r,c,g,s,a
2) DM manager: if the model is g,s,a
'''
datasets, dataloaders = DT_manager.get_train_val_test_data(mdp_constr=mdp_constr, timestep_norm=False)
train_loader, eval_loader, test_loader = dataloaders
train_dataset, val_dataset, test_dataset = datasets
# print('Length of the test loader:',len(test_loader))
...
'''
SSM based warm start
'''

# Import the SSM
model = DM_manager.get_DM_model(model_name, train_loader, eval_loader)
print(model)
model.eval()
inference_func = getattr(DM_manager, 'ssm_model_inference_'+transformer_ws)
